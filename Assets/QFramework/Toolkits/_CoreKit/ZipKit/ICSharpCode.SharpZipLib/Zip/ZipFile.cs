    ...
    public class ZipFile : IEnumerable, IDisposable
    {
        ...
        */
        ...
        private void ReadEntries()
        {
            ...
            if (!baseStream_.CanSeek) throw new ZipException("ZipFile stream must be seekable");
...
#pragma warning restore 0219
        ...
        private Stream CreateAndInitDecryptionStream(Stream baseStream, ZipEntry entry)
        {
                ...
                || (entry.Flags & (int)GeneralBitFlags.StrongEncryption) == 0)
            {
                ...
                if (!HaveKeys) throw new ZipException("No password available for encrypted stream");
            ...
            else
            {
                if (entry.Version == ZipConstants.VERSION_AES)
                {
                    //
                    OnKeysRequired(entry.Name);
                    if (!HaveKeys) throw new ZipException("No password available for AES encrypted stream");
