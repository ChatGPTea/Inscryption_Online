...
namespace MoonSharp.Interpreter
{
 ...
 mi.CreateDelegate(typeof(Func<ScriptExecutionContext, CallbackArguments, DynValue>));
            ...
            foreach (var fi in Framework.Do.GetFields(t).Where(_mi =>
                         _mi.IsStatic && _mi.GetCustomAttributes(typeof(MoonSharpModuleMethodAttribute), false).Length >
                         0))
            {
            ...
            foreach (var fi in Framework.Do.GetFields(t).Where(_mi =>
                         _mi.IsStatic &&
                         _mi.GetCustomAttributes(typeof(MoonSharpModuleConstantAttribute), false).Length > 0))
            {
