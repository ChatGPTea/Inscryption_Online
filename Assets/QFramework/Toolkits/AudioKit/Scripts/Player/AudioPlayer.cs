...
namespace QFramework
{
    public class AudioPlayer : IPoolable, IPoolType, IAudioKitOnFinish, IAudioKitOnRelease
    {
        ...
        private void AllocateAudioSourceController(GameObject root, string name)
        {
        ...
        {
            if (clip == null || GetName == name) return;
        ...
        private void OnResLoadFinish(bool result, AudioClip clip)
        {
            ...
            if (mAudioClip == null)
            {
                Debug.LogError("Asset Is Invalid AudioClip:" + GetName);
                Release();
                return;
            }
