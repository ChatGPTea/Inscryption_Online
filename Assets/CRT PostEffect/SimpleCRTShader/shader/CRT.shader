    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                /////Scanline 
                float scanline = sin(i.uv.y * 800.0); // 静止扫描线，无 Time
                float scanlineDark = max(0.0, -scanline); // 只取波谷部分（负变正）
                scanlineDark *= 0.04; // 控制暗线强度
                color.rgb -= scanlineDark * _ScanlineOnOff; // 真正的暗线效果
                ...
                float dist = distance(i.uv, center); // 当前 UV 到屏幕中心的距离
                float vignette = 1.0 - smoothstep(0.2, 0.9, dist); // 0.3 ~ 0.8 控制暗角范围与强度

                // 可选：让中间更亮（比如乘以 >1.0 的值），四角更暗
                // 例如：vignette = 0.7 + 0.3 * vignette; // 增强对比
                // 或者直接用： vignette = smoothstep(0.2, 0.9, dist); 然后 1.0 - vignette

                color.rgb *= vignette; // 将亮度遮罩应用到最终颜色
                /////
