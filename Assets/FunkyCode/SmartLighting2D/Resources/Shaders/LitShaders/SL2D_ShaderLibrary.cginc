...
bool InGameCamera(int id, float2 worldPos)
{
    ...
    return (InCamera(TransformToCamera(worldPos - float2(rect.x, rect.y), GetLightmapRotation(id)),
...
bool InSceneCamera(int id, float2 worldPos)
{
    ...
    return (InCamera(TransformToCamera(worldPos - float2(rect.x, rect.y), GetSceneRotation(id)),
                     float2(rect.z, rect.w)));
}
...
float3 SL2D_DayBump(float2 uv)
{
    ...
    {
        float specular = _LightIntensity * pow(
