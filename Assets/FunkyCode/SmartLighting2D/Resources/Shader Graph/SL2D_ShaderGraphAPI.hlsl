...
sampler2D _GameTexture1;
float4 _GameRect1;
float4 _GameColor1;
float _GameRotation1;
sampler2D _GameTexture2;
float4 _GameRect2;
float4 _GameColor2;
float _GameRotation2;
sampler2D _GameTexture3;
float4 _GameRect3;
float4 _GameColor3;
float _GameRotation3;
sampler2D _GameTexture4;
float4 _GameRect4;
float4 _GameColor4;
float _GameRotation4;

// scene lightmaps
sampler2D _SceneTexture1;
float4 _SceneRect1;
float4 _SceneColor1;
float _SceneRotation1;
sampler2D _SceneTexture2;
float4 _SceneRect2;
float4 _SceneColor2;
float _SceneRotation2;
sampler2D _SceneTexture3;
float4 _SceneRect3;
float4 _SceneColor3;
float _SceneRotation3;
sampler2D _SceneTexture4;
float4 _SceneRect4;
float4 _SceneColor4;
float _SceneRotation4;
...
bool internal_InCamera(float2 pos, float2 rectSize)
{
    ...
    return !(pos.x < rectPos.x || pos.x > rectPos.x + rectSize.x || pos.y < rectPos.y || pos.y > rectPos.y + rectSize.
        y);
}
...
{
    color = lerp(float4(1, 1, 1, 1), float4(1 - strength, 1 - strength, 1 - strength, 1),
                 step(depth, SL2D_Pass_Depth(1, world)));
    color = min(color, lerp(float4(1, 1, 1, 1), float4(1 - strength, 1 - strength, 1 - strength, 1),
                            step(depth, SL2D_Pass_Depth(2, world))));
    color = min(color, lerp(float4(1, 1, 1, 1), float4(1 - strength, 1 - strength, 1 - strength, 1),
                            step(depth, SL2D_Pass_Depth(3, world))));
    color = min(color, lerp(float4(1, 1, 1, 1), float4(1 - strength, 1 - strength, 1 - strength, 1),
                            step(depth, SL2D_Pass_Depth(4, world))));
}
...
void SL2D_Depth_1_float(float2 world, float depth, float strength, out float4 color)
{
    color = lerp(float4(1, 1, 1, 1), float4(1 - strength, 1 - strength, 1 - strength, 1),
                 step(depth, SL2D_Pass_Depth(1, world)));
}

void SL2D_Depth_2_float(float2 world, float depth, float strength, out float4 color)
{
    color = lerp(float4(1, 1, 1, 1), float4(1 - strength, 1 - strength, 1 - strength, 1),
                 step(depth, SL2D_Pass_Depth(2, world)));
}

void SL2D_Depth_3_float(float2 world, float depth, float strength, out float4 color)
{
    color = lerp(float4(1, 1, 1, 1), float4(1 - strength, 1 - strength, 1 - strength, 1),
                 step(depth, SL2D_Pass_Depth(3, world)));
}

void SL2D_Depth_4_float(float2 world, float depth, float strength, out float4 color)
{
    color = lerp(float4(1, 1, 1, 1), float4(1 - strength, 1 - strength, 1 - strength, 1),
                 step(depth, SL2D_Pass_Depth(4, world)));
}
...
void SL2D_DayBump_float(float2 uv, float intensity, float specular, out float4 color)
{
    ...
    {
        float spec = intensity * pow(
            max(0.0, dot(reflect(-lightDirection, normalDirection), float3(0.0f, 0.0f, -1.0f))), 10);
