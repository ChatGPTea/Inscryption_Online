...
namespace FunkyCode
{
    ...
    public class LightTilemapOcclusion2D : MonoBehaviour
    {
        public enum MapType
        {
            UnityEngineTilemapRectangle
        }

        private static OcclusionTileset tileset;
        ...
        private OcclusionMesh tilemapMesh;
        ...
        {
            if (tilemap2D == null) return;

            SetupMap();

            if (tileset == null) tileset = OcclusionTileset.Load("Textures/OclussionMap");
        ...
        public void SetupMap()
        {
            ...
            var tilemap = (ITilemap)FormatterServices.GetUninitializedObject(typeof(ITilemap));
            typeof(ITilemap).GetField("m_Tilemap", BindingFlags.NonPublic | BindingFlags.Instance)
                .SetValue(tilemap, tilemap2D);
            ...
            foreach (var position in tilemap2D.cellBounds.allPositionsWithin)
            {
                ...
                if (tilebase != null)
                {
                    ...
                    if (onlyColliders && tileData.colliderType == Tile.ColliderType.None) continue;

                    var tile = new RectangleTile();
        ...
        public void GenerateMesh()
        {
            ...
            {
                if (map.GetUnwalkableInt(x, y) == 1) continue;
                ...
                var position = new Vector2Int(x - map.width / 2, y - map.height / 2);

                if (x1y1 == 1 && x0y1 + x1y0 == 0) AddTile(8, position, OcclusionTileset.TileRotation.up);
                if (x9y9 == 1 && x0y9 + x9y0 == 0) AddTile(8, position, OcclusionTileset.TileRotation.down);
                if (x9y1 == 1 && x0y1 + x9y0 == 0) AddTile(8, position, OcclusionTileset.TileRotation.right);
                if (x1y9 == 1 && x0y9 + x1y0 == 0) AddTile(8, position, OcclusionTileset.TileRotation.left);
                ...
                {
                    AddTile(4, position, OcclusionTileset.TileRotation.up);
                    continue;
                }

                if (x0y1 == 1 && x1y0 == 1 && x1y9 == 1 && x9y1 + x9y0 + x0y9 == 0)
                {
                    AddTile(4, position, OcclusionTileset.TileRotation.right, true);
                    continue;
                }

                if (x0y9 == 1 && x9y0 == 1 && x1y9 == 1 && x9y1 + x1y0 + x0y1 == 0)
                {
                    AddTile(4, position, OcclusionTileset.TileRotation.down);
                    continue;
                }

                if (x0y9 == 1 && x9y0 == 1 && x9y1 == 1 && x1y9 + x1y0 + x0y1 == 0)
                {
                    AddTile(4, position, OcclusionTileset.TileRotation.left, true);
                    continue;
                }

                if (x9y0 == 1 && x0y1 == 1 && x1y1 == 1 && x9y9 + x0y9 + x1y0 == 0)
                {
                    AddTile(4, position, OcclusionTileset.TileRotation.up, true);
                    continue;
                }

                if (x1y0 == 1 && x0y9 == 1 && x1y1 == 1 && x9y9 + x0y1 + x9y0 == 0)
                {
                    AddTile(4, position, OcclusionTileset.TileRotation.left);
                    continue;
                }

                if (x1y0 == 1 && x0y9 == 1 && x9y9 == 1 && x1y1 + x0y1 + x9y0 == 0)
                {
                    AddTile(4, position, OcclusionTileset.TileRotation.down, true);
                    continue;
                }

                if (x9y0 == 1 && x0y1 == 1 && x9y9 == 1 && x1y1 + x0y9 + x1y0 == 0)
                {
                    AddTile(4, position, OcclusionTileset.TileRotation.right);
                    continue;
                }

                if (x0y1 == 1 && x9y0 + x1y0 + x1y1 + x9y1 == 0) AddTile(5, position, OcclusionTileset.TileRotation.up);
                ...
                if (x0y1 == 1 && x1y1 == 1 && x9y0 + x1y0 + x9y1 == 0)
                    AddTile(7, position, OcclusionTileset.TileRotation.up);
                if (x0y1 == 1 && x9y1 == 1 && x9y0 + x1y0 + x1y1 == 0)
                    AddTile(7, position, OcclusionTileset.TileRotation.up, true);
                if (x0y9 == 1 && x9y9 == 1 && x1y0 + x9y0 + x1y9 == 0)
                    AddTile(7, position, OcclusionTileset.TileRotation.down);
                if (x0y9 == 1 && x1y9 == 1 && x1y0 + x9y0 + x9y9 == 0)
                    AddTile(7, position, OcclusionTileset.TileRotation.down, true);
                ...
                if (x1y0 == 1 && x1y9 == 1 && x0y9 + x0y1 + x1y1 == 0)
                    AddTile(7, position, OcclusionTileset.TileRotation.left);
                if (x9y0 == 1 && x9y1 == 1 && x0y9 + x0y1 + x9y9 == 0)
                    AddTile(7, position, OcclusionTileset.TileRotation.right);
                if (x9y0 == 1 && x9y9 == 1 && x0y9 + x0y1 + x9y1 == 0)
                    AddTile(7, position, OcclusionTileset.TileRotation.left, true);

                // SIMPLE UNFINISHED
                if (x0y1 == 1 && x1y1 == 1 && x9y1 == 1 && x9y0 + x1y0 == 0)
                    AddTile(6, position, OcclusionTileset.TileRotation.up, true);
                if (x0y9 == 1 && x1y9 == 1 && x9y9 == 1 && x1y0 + x9y0 == 0)
                    AddTile(6, position, OcclusionTileset.TileRotation.down, true);
                if (x1y0 == 1 && x1y1 == 1 && x1y9 == 1 && x0y9 + x0y1 == 0)
                    AddTile(6, position, OcclusionTileset.TileRotation.right, true);
                if (x9y0 == 1 && x9y1 == 1 && x9y9 == 1 && x0y9 + x0y1 == 0)
                    AddTile(6, position, OcclusionTileset.TileRotation.left, true);

                if (x0y1 == 1 && x9y0 == 1 && x0y9 == 1 && x1y9 == 1 && x1y1 + x1y0 == 0)
                {
                    AddTile(9, position, OcclusionTileset.TileRotation.up);
                    continue;
                }

                if (x1y0 == 1 && x0y1 == 1 && x9y0 == 1 && x9y9 == 1 && x1y9 + x0y9 == 0)
                {
                    AddTile(9, position, OcclusionTileset.TileRotation.left);
                    continue;
                }

                if (x9y0 == 1 && x0y9 == 1 && x1y0 == 1 && x1y1 == 1 && x9y1 + x0y1 == 0)
                {
                    AddTile(9, position, OcclusionTileset.TileRotation.right);
                    continue;
                }

                if (x0y9 == 1 && x1y0 == 1 && x0y1 == 1 && x9y1 == 1 && x9y9 + x9y0 == 0)
                {
                    AddTile(9, position, OcclusionTileset.TileRotation.down);
                    continue;
                }
        ...
        private void AddTile(int id, Vector2Int tilePosition, OcclusionTileset.TileRotation tileRotation,
        ...
        private void ExportMesh()
        {
            ...
            {
                var material = new Material(Shader.Find("Light2D/Internal/Multiply HDR"));
        ...
        public class RectangleMap
        {
            ...
            public List<RectangleTile> mapTiles = new();
            public int width;
