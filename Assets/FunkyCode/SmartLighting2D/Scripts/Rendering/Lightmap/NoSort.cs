...
namespace FunkyCode.Rendering.Lightmap
{
    public class NoSort
    {
        ...
        private static void DrawRooms(Pass pass)
        {
            ...
            if (Room.drawColliderPass) GL.End();
        ...
        private static void DrawTilemapRooms(Pass pass)
        {
            ...
            var roomTilemapCount = roomTilemapList.Count;

            if (roomTilemapCount < 1)
                return;

            for (var i = 0; i < roomTilemapCount; i++)
            {
        ...
        private static void DrawLightSprites(Pass pass)
        {
            ...
            var spriteRendererCount = spriteRendererList.Count;
            ...
            // Draw Simple
            for (var i = 0; i < spriteRendererCount; i++)
            {
            ...
            if (LightSprite.Pass.currentTexture != null) GL.End();
        }
        ...
        private static void DrawLightTextures(Pass pass)
        {
            ...
            var lightTextureCount = lightTextureList.Count;

            if (lightTextureCount < 1)
                return;

            for (var i = 0; i < lightTextureCount; i++)
            {
        ...
        private static void DrawLightParticleSystem(Pass pass)
        {
            ...
            var lightParticleSystemCount = particleRendererList.Count;

            if (lightParticleSystemCount < 1)
                return;

            for (var i = 0; i < lightParticleSystemCount; i++)
            {
        ...
        private static void DrawLight(Pass pass)
        {
            ...
            {
                var id = lightList[i];
                if (id.lightLayer >= 0 && id.lightLayer == pass.layerId) LightSource.Draw(id, pass.camera);
