...
namespace FunkyCode.Rendering.Lightmap
{
    ...
    {
        public static bool drawColliderPass;
        ...
        public static void DrawColliderPass(LightRoom2D id, Camera camera)
        {
            ...
            GLExtended.DrawMeshPass(meshObjects, position, id.transform.lossyScale,
                id.transform.rotation.eulerAngles.z);
        }

        public static void DrawCollider(LightRoom2D id, Camera camera)
        {
            var meshObjects = id.shape.GetMeshes();
            if (meshObjects == null)
                return;
        ...
        public static void DrawSprite(LightRoom2D id, Camera camera)
        {
            ...
            GLExtended.color = id.color;
            Vector2 position = id.transform.position - camera.transform.position;
            Sprite.FullRect.Draw(id.spriteMeshObject, spriteRenderer, position, id.transform.lossyScale,
                id.transform.eulerAngles.z);
        }
    }
}