...
namespace FunkyCode.Rendering.Light
{
    public class Mesh
    {
        public static void Mask(Light2D light, LightCollider2D id, Material material, LayerSetting layerSetting)
        {
            ...
            var position = id.mainShape.transform2D.Position - light.transform2D.position;
            var pivotPosition = id.mainShape.GetPivotPoint() - light.transform2D.position;
            GLExtended.color = LayerSettingColor.Get(pivotPosition, layerSetting, id.maskLit, 1, id.maskLitCustom);

            material.SetPass(0);

            GLExtended.DrawMesh(meshObjects, position, id.mainShape.transform2D.Scale,
                id.mainShape.transform2D.Rotation);

            material.mainTexture = null;
        }

        public static void MaskNormalMap(Light2D light, LightCollider2D id, Material material,
            LayerSetting layerSetting)
        {
            ...
            var color = material.color.r;

            switch (id.bumpMapMode.type)
            {
                case NormalMapType.ObjectToLight:

                    rotation = Mathf.Atan2(light.transform2D.position.y - id.mainShape.transform2D.Position.y,
                        light.transform2D.position.x - id.mainShape.transform2D.Position.x);
                ...
                case NormalMapType.PixelToLight:
                    ...
                    var sc = id.mainShape.transform2D.Scale;
                    sc = sc.normalized;
