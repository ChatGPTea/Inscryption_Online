...
namespace FunkyCode.Rendering.Light
{
    public static class SpriteRenderer2D
    {
        ...
        public static void Mask(Light2D light, LightCollider2D id, Material material, LayerSetting layerSetting)
        {
            ...
            GLExtended.color =
                LayerSettingColor.Get(shape, localPosition, layerSetting, id.maskLit, 1, id.maskLitCustom);

            Sprite.Pass.Draw(id.spriteMeshObject, spriteRenderer, localPosition, shape.transform2D.Scale,
        ...
        public static void MaskBumped(Light2D light, LightCollider2D id, Material material, LayerSetting layerSetting)
        {
            ...
            var position = shape.transform2D.Position - light.transform2D.position;

            material.mainTexture = sprite.texture;
            GLExtended.color = LayerSettingColor.Get(position, layerSetting, id.maskLit, 1, id.maskLitCustom);

            var color = GLExtended.color.r;

            switch (id.bumpMapMode.type)
            {
                case NormalMapType.ObjectToLight:

                    rotation = Mathf.Atan2(light.transform2D.position.y - shape.transform2D.Position.y,
                        light.transform2D.position.x - shape.transform2D.Position.x);
                ...
                case NormalMapType.PixelToLight:
                    ...
                    var sc = shape.transform2D.Scale.normalized;
            ...
            Sprite.Draw(id.spriteMeshObject, spriteRenderer, position, shape.transform2D.Scale,
                shape.transform2D.Rotation);
        }
    }
}