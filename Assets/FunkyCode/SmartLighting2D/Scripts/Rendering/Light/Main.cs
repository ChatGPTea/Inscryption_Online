...
namespace FunkyCode.Rendering.Light
{
    public static class Main
    {
        ...
        public static void Draw(Light2D light)
        {
            ...
            var layerSettings = light.GetLightPresetLayers();

            if (layerSettings == null) return;

            if (layerSettings.Length < 1) return;

            for (var layerID = 0; layerID < layerSettings.Length; layerID++)
            {
                var layerSetting = layerSettings[layerID];

                if (layerSetting == null) continue;

                if (!pass.Setup(light, layerSetting)) continue;
        ...
        public static void DrawTranslucency(Light2D light)
        {
            ...
            var layerSettings = light.GetTranslucencyPresetLayers();
