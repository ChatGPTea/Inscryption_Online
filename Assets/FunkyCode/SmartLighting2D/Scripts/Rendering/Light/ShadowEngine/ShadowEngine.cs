...
namespace FunkyCode.Rendering.Light
{
    ...
    public static class ShadowEngine
    {
        ...
        public static float lightSize;

        public static bool continueDrawing;
        public static bool ignoreInside;
        public static bool dontdrawInside;
        ...
        // Layer Effect
        public static List<List<Polygon2>> effectPolygons = new();

        // public static float shadowDistance;

        public static bool softShadow;
        ...
        public static void SetPass(Light2D lightObject, LayerSetting layer)
        {
            ...
            softShadow = layer.shadowEffect == LightLayerShadowEffect.SoftConvex ||
                         layer.shadowEffect == LightLayerShadowEffect.SoftVertex;
        ...
        {
            var layerID = effectLayer;

            foreach (var c in LightCollider2D.GetShadowList(layerID))
            {
                var polygons = c.mainShape.GetPolygonsWorld();

                if (polygons == null) continue;

                if (c.InLight(light)) effectPolygons.Add(polygons);
            }
        }
