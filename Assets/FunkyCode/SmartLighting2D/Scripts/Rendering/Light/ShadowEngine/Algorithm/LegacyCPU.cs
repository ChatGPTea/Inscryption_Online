...
namespace FunkyCode.Rendering.Light.Shadow
{
    public static class LegacyCPU
    {
            ...
            float translucency)
        {
            ...
            var lightSizeSquare = ShadowEngine.lightSize * ShadowEngine.lightSize * 0.5f;
            ...
            if (translucency > 0) GL.Color(new Color(1 - translucency, 0, 0, 0));
            ...
            for (var i = 0; i < PolygonCount; i++)
            {
                ...
                for (var x = 0; x < pointsCount; x++)
                {
                    ...
                    if (closestPoint.x * closestPoint.x > lightSizeSquare ||
                        closestPoint.y * closestPoint.y > lightSizeSquare) continue;
                    ...
                    */
                    ...
                    if (outerAngle > 0)
                    {
                        angleA = (float)Math.Atan2(edgeAWorld.y, edgeAWorld.x);
                        angleB = (float)Math.Atan2(edgeBWorld.y, edgeBWorld.x);
            ...
            if (translucency > 0) GL.Color(new Color(1, 0, 1, 1));
        }
    }
}