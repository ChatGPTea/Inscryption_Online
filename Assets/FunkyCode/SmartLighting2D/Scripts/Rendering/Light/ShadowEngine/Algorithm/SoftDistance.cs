...
namespace FunkyCode.Rendering.Light.Shadow
{
    public static class SoftDistance
    {
            ...
            float translucency)
        {
            ...
            var position = ShadowEngine.lightOffset + ShadowEngine.objectOffset;

            var PolygonCount = polygons.Count;

            var draw = ShadowEngine.drawOffset;

            var finiteShadows = true;
            ...
            for (var i = 0; i < PolygonCount; i++)
            {
                ...
                for (var x = 0; x < pointsCount; x++)
                {
                    ...
                    var mx = (edgeAWorld.x + edgeBWorld.x) / 2;
                    var my = (edgeAWorld.y + edgeBWorld.y) / 2;

                    var step = Mathf.Sqrt(mx * mx + my * my) / ShadowEngine.lightSize;
                    var shadowLength = Mathf.Lerp(shadowDistanceMin, shadowDistanceMax, step);
                    ...
                    {
                        // Detailed Shadow
                        var closestPoint = Math2D.ClosestPointOnLine(projectedLeft, projectedRight);
        ...
        {
            var stepNext = nextPoint.magnitude / ShadowEngine.lightSize;
            var sizePointNext = Mathf.Lerp(ShadowEngine.light.shadowDistanceClose, ShadowEngine.light.shadowDistanceFar,
                stepNext);

            var step = point.magnitude / ShadowEngine.lightSize;
            var sizePoint = Mathf.Lerp(ShadowEngine.light.shadowDistanceClose, ShadowEngine.light.shadowDistanceFar,
                step);
