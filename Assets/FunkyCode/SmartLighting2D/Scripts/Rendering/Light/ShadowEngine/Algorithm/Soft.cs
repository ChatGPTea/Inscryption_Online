...
namespace FunkyCode.Rendering.Light.Shadow
{
    public static class Soft
    {
        ...
        private static readonly EdgePass pass = new();
        ...
        private static void ShadowForVertex(Polygon2 polygon, Vector2 position)
        {
            if (ShadowEngine.ignoreInside)
            {
                // change to sides of vertices?
                if (Math2D.PointInPoly(-position, polygon)) return;
            }
            ...
            for (var x = 0; x < pointsCount; x++)
            {
                ...
                edgePosition.x = (pair.A.x + pair.B.x) / 2;
                edgePosition.y = (pair.A.y + pair.B.y) / 2;

                var edgeRotation = (float)Math.Atan2(pair.B.y - pair.A.y, pair.B.x - pair.A.x);

                var edgeSize = Vector2.Distance(pair.A, pair.B) / 2;
        ...
        private static void ShadowForObject(Polygon2 polygon, Vector2 position, float shadowTranslucency)
        {
            if (ShadowEngine.ignoreInside)
            {
                // change to sides of vertices?
