    ...
    public static class Main
    {
        ...
        private static void UpdateLoop()
        {
            if (DayLightCollider2D.List.Count > 0)
                for (var id = 0; id < DayLightCollider2D.List.Count; id++)
                    DayLightCollider2D.List[id].UpdateLoop();

            if (LightCollider2D.List.Count > 0)
                for (var id = 0; id < LightCollider2D.List.Count; id++)
                    LightCollider2D.List[id].UpdateLoop();

            if (LightSprite2D.List.Count > 0)
                for (var id = 0; id < LightSprite2D.List.Count; id++)
            ...
            if (OnRenderMode.List.Count > 0)
                for (var id = 0; id < OnRenderMode.List.Count; id++)
        ...
        public static void UpdateCameras()
        {
            ...
            for (var i = 0; i < cameras.Length; i++)
            {
                ...
                for (var b = 0; b < cameraSetting.Lightmaps.Length; b++)
                {
                    ...
                    if (cameraLightmap.sceneView == CameraLightmap.SceneView.Enabled)
                    {
                        ...
                        if (buffer != null)
                        {
                            var camera = sceneSettings.GetCamera();
                            CameraTransform.GetCamera(camera);
            ...
            if (MaterialSystem.Count > 0)
            {
                ...
                for (var i = 0; i < MaterialSystem.Count; i++)
                {
                    ...
                    if (passId == 0) passId = 1; // incremental
        ...
        public static void PassLightmap(bool isSceneView, LightMainBuffer2D buffer, CameraSettings cameraSetting,
            CameraLightmap cameraLightmap, LightmapPreset lightmapPreset)
        {
            ...
            switch (cameraLightmap.output)
            {
                case CameraLightmap.Output.Materials:
                    ...
                    foreach (var material in cameraLightmap.GetMaterials().materials)
                    {
                        ...
                        MaterialSystem.Add(material, isSceneView, matPassId, camera, buffer.renderTexture,
                            lightmapPreset);
                    }
        ...
        {
            if (LightMainBuffer2D.List.Count <= 0) return;

            for (var i = 0; i < LightMainBuffer2D.List.Count; i++) LightMainBuffer2D.List[i]?.Update();

            for (var i = 0; i < LightMainBuffer2D.List.Count; i++)
            {
                ...
                var render = cameraLightmap.rendering != CameraLightmap.Rendering.Disabled;
