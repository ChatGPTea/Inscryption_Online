        ...
        public static void Draw(Pass pass)
        {
            for (var id = 0; id < pass.sortList.Count; id++)
            {
                ...
                if (lightObject is DayLightCollider2D dayCollider)
                {
                    ...
                    if (pass.drawShadows)
                    {
                        if (dayCollider.mainShape.shadowType == DayLightCollider2D.ShadowType.Collider2D ||
                            dayCollider.mainShape.shadowType == DayLightCollider2D.ShadowType.SpritePhysicsShape)
                        {
                    ...
                    if (pass.drawMask) SpriteRenderer2D.Draw(dayCollider, pass.offset);
                }
