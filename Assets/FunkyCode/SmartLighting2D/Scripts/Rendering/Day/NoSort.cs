...
namespace FunkyCode.Rendering.Day
{
    public static class NoSort
    {
        ...
        {
            var drawShadows = pass.layer.type != LayerType.MaskOnly;
            var drawMask = pass.layer.type != LayerType.ShadowsOnly;
        ...
        public static class Mask
        {
            ...
            {
                if (pass.colliderCount <= 0) return;

                // regular sprites

                SpriteRenderer2D.currentTexture = null;

                for (var i = 0; i < pass.colliderCount; i++)
                {
                    var id = pass.colliderList[i];

                    if (id.maskLayer != pass.layerId) continue;
            ...
            {
                if (pass.tilemapColliderCount <= 0) return;

                for (var i = 0; i < pass.tilemapColliderCount; i++)
                {
                    var id = pass.tilemapColliderList[i];
        ...
        public static class Shadow
        {
            public static void DrawColliderFill(Pass pass)
            {
                ...
                for (var i = 0; i < pass.colliderCount; i++)
                {
                    ...
                    if (id.shadowLayer != pass.layerId) continue;
            ...
            public static void DrawSprite(Pass pass)
            {
                ...
                if (SpriteRendererShadow.currentTexture != null) SpriteRendererShadow.End();
            }
        }
    }
}