...
namespace FunkyCode.EventHandling
{
    public class Object
    {
        public List<LightCollider2D> listenersCache = new();

        public List<LightCollision2D> listenersInLight = new();
        public List<LightCollider2D> listenersInLightColliders = new();
        ...
        public void Update(Light2D light, EventPreset eventPreset)
        {
            ...
            {
                for (var i = 0; i < listenersCache.Count; i++)
                {
            ...
            foreach (var collision in listenersInLight) listenersInLightColliders.Add(collision.collider);
            ...
            for (var i = 0; i < listenersInLight.Count; i++)
            {
