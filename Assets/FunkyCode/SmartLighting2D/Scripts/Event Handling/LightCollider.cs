...
namespace FunkyCode.EventHandling
{
    public class LightCollider : Base
    {
        ...
        public static void GetCollisions(List<LightCollision2D> collisions, Light2D lightingSource)
        {
            ...
            for (var i = 0; i < colliderList.Count; i++)
            {
                ...
                foreach (var point in polygon.points)
                {
                    ...
                    {
                        var direction = p.Atan2(Vector2.zero) * Mathf.Rad2Deg;
                        ...
                        if (direction <= lightingSource.spotAngleInner / 2 ||
                            direction >= 360 - lightingSource.spotAngleInner / 2) collision.points.Add(p);
                    }
                }

                if (collision.points.Count > 0) collisions.Add(collision);
            }
        }
        ...
        {
            var lightSize = Mathf.Sqrt(light.size * light.size + light.size * light.size);
            ...
            for (var iid = 0; iid < eventPreset.layerSetting.list.Length; iid++)
            {
                ...
                var colliderCount = colliderList.Count;

                for (var ci = 0; ci < colliderCount; ci++)
                {
                    ...
                    for (var i = 0; i < polygons.Count; i++)
                    {
                        ...
                        for (var x = 0; x < pointsCount; x++)
                        {
                            ...
                            projectionLeft.x = edgeLeft.x + (float)Math.Cos(rotLeft) * lightSize;
                            projectionLeft.y = edgeLeft.y + (float)Math.Sin(rotLeft) * lightSize;

                            projectionRight.x = edgeRight.x + (float)Math.Cos(rotRight) * lightSize;
                            projectionRight.y = edgeRight.y + (float)Math.Sin(rotRight) * lightSize;
