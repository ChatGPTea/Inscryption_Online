...
namespace FunkyCode.LightShape
{
    public class SpriteShape : Base
    {
        ...
        private readonly VirtualSpriteRenderer virtualSpriteRenderer = new();
        ...
        public override List<Polygon2> GetPolygonsLocal()
        {
            if (LocalPolygons == null)
            {
                ...
                {
                    Debug.LogWarning("Light Collider 2D: Cannot access sprite renderer (Sprite Shape Local)",
                    ...
                    spriteRenderer.tileMode == SpriteTileMode.Continuous)
                {
                    ...
                    var size = transform.localScale * spriteRenderer.size * 0.5f;
                    ...
                    v1 = new Vector2(pos.x + Mathf.Cos(rectAngle + rot) * dist,
                        pos.y + Mathf.Sin(rectAngle + rot) * dist);
                    v2 = new Vector2(pos.x + Mathf.Cos(-rectAngle + rot) * dist,
                        pos.y + Mathf.Sin(-rectAngle + rot) * dist);
                    v3 = new Vector2(pos.x + Mathf.Cos(rectAngle + Mathf.PI + rot) * dist,
                        pos.y + Mathf.Sin(rectAngle + Mathf.PI + rot) * dist);
                    v4 = new Vector2(pos.x + Mathf.Cos(-rectAngle + Mathf.PI + rot) * dist,
                        pos.y + Mathf.Sin(-rectAngle + Mathf.PI + rot) * dist);
                }
                else
                {
                    ...
                    var spriteTransform = new SpriteTransform(virtualSpriteRenderer, position, scale, rotation);
        ...
        public override List<Polygon2> GetPolygonsWorld()
        {
            if (WorldPolygons == null)
            {
                ...
                if (WorldCache == null)
                {
                    ...
                    if (spriteRenderer == null)
                    {
                        Debug.LogWarning("Light Collider 2D: Cannot access sprite renderer (Sprite Shape)",
                            transform.gameObject);
                    ...
                    {
                        var rot = transform.eulerAngles.z;
                        var size = transform.lossyScale * spriteRenderer.size * 0.5f;
