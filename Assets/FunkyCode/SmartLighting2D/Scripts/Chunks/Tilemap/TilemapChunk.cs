...
namespace FunkyCode.Chunks
{
    public class TilemapManager
    {
        ...
        private void GenerateChunks(Rect worldRect)
        {
            ...
            if (tp0.x > tp1.x) tp1.x += 100;

            if (tp0.y > tp1.y) tp1.y += 100;

            for (var x = tp0.x; x <= tp1.x; x++)
            for (var y = tp0.y; y <= tp1.y; y++)
            {
                var tx = x % 100;
                var ty = y % 100;

                if (maps[tx, ty] == null) continue;
        ...
        public static Vector2Int Transform(Vector2 position)
        {
            var tx = position.x / ChunkSize;
            var ty = position.y / ChunkSize;
        ...
        public static Vector2Int TransformBounds(Vector2 position)
        {
            var tx = position.x / ChunkSize;
            var ty = position.y / ChunkSize;
