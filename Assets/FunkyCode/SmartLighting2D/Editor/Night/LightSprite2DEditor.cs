...
namespace FunkyCode
{
    ...
    public class LightSprite2DEditor : Editor
    {
        ...
        private LightSprite2D lightSprite2D;

        private SerializedProperty sprite;
        private SerializedProperty spriteMode;
        private SerializedProperty transform_applyRotation;
        private SerializedProperty transform_position;
        private SerializedProperty transform_rotation;

        private SerializedProperty transform_scale;
        ...
        {
            lightLayer.intValue = EditorGUILayout.Popup("Layer (Light)", lightLayer.intValue,
        ...
        private void DrawSpriteRenderer(LightSprite2D script)
        {
            if (script.spriteMode == LightSprite2D.SpriteMode.Custom)
            {
                var foldout0 = GUIFoldout.Draw("Sprite Renderer", script);

                if (foldout0)
                {
                    EditorGUI.indentLevel++;

                    sprite.objectReferenceValue = (Sprite)EditorGUILayout.ObjectField("Sprite",
                        sprite.objectReferenceValue, typeof(Sprite), true);
        ...
        {
            var colorValue = lightSprite2D.color;
        ...
        private void DrawTransform(LightSprite2D script)
        {
            var foldout = GUIFoldout.Draw("Transform", script.lightSpriteTransform);
