...
namespace FunkyCode.Utilities.Polygon2DTriangulation
{
    public static class DTSweep
    {
            ...
            DelaunayTriangle t, TriangulationPoint p)
        {
            ...
            if (ot == null) throw new InvalidOperationException("[BUG:FIXME] FLIP failed due to missing triangle");
            ...
            if (inScanArea)
            {
                ...
                if (p == eq && op == ep)
                {
                    ...
                    {
                        if (tcx.IsDebugEnabled) Debug.Log("[FLIP] - constrained edge done");
                    ...
                    {
                        if (tcx.IsDebugEnabled) Debug.Log("[FLIP] - subedge done");
                    }
                }
                else
                {
            ...
            DelaunayTriangle flipTriangle, DelaunayTriangle t, TriangulationPoint p)
        {
            ...
            if (ot == null) throw new Exception("FLIP failed due to missing triangle");

            if (tcx.IsDebugEnabled)
            {
                Debug.Log("scan next point");
                tcx.DTDebugContext.PrimaryTriangle = t;
                tcx.DTDebugContext.SecondaryTriangle = ot;
            }
            ...
            {
                if (NextFlipPoint(ep, eq, ot, op, out newP)) FlipScanEdgeEvent(tcx, ep, eq, flipTriangle, ot, newP);
            }
        }
