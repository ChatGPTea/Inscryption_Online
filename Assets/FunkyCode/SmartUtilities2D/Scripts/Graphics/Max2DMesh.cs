...
namespace FunkyCode.Utilities
{
    ...
    {
        public List<Mesh2DSubmesh> submeshes = new();
    ...
    public class Max2DMesh
    {
        ...
        private const float uv0 = 1f / 32;
        private const float uv1 = 1f - uv0;

        private static readonly Vector2D A1 = Vector2D.Zero();
        private static readonly Vector2D A2 = Vector2D.Zero();
        private static readonly Vector2D B1 = Vector2D.Zero();
        private static readonly Vector2D B2 = Vector2D.Zero();

        private static readonly Vector2D A3 = Vector2D.Zero();
        private static readonly Vector2D A4 = Vector2D.Zero();

        private static readonly Pair2D pair2D = Pair2D.Zero();

        public static void Draw(Mesh mesh, Transform transform, Material material)
        {
        ...
        {
            var matrix = Matrix4x4.TRS(Vector3.zero, Quaternion.Euler(0, 0, 0), new Vector3(1, 1, 1));

            Graphics.DrawMesh(mesh, matrix, material, 0);
        }

        public static Mesh CreatePolygon(Transform transform, Polygon2D polygon, float lineOffset, float lineWidth,
            bool connectedLine)
        {
            var trianglesList = new Mesh2DMesh();

            var count = polygon.pointsList.Count;
            var lastID = count - 1;
            var startID = 0;
            ...
            for (var i = startID; i < count; i++)
            {
                ...
                trianglesList.Add(CreateLine(p, transform.localScale, lineWidth, lineOffset));

                p.A = p.B;
            }

            foreach (var hole in polygon.holesList)
            {
        ...
        public static Mesh2DSubmesh CreateLine(Pair2D pair, Vector3 transformScale, float lineWidth, float z = 0f)
        {
            var result = new Mesh2DSubmesh(18);
            ...
            var size = lineWidth / 6;
            var rot = (float)Vector2D.Atan2(pair.A, pair.B);
            ...
            var scale = new Vector2(1f / transformScale.x, 1f / transformScale.y);
        ...
        public static Mesh Export(Mesh2DMesh trianglesList)
        {
            var mesh = new Mesh();
            var vertices = new Vector3[trianglesList.verticesCount];
            var uv = new Vector2[trianglesList.verticesCount];
            var triangles = new int[trianglesList.verticesCount];
            ...
            for (var x = 0; x < trianglesList.submeshes.Count; x++)
            {
                triangle = trianglesList.submeshes[x];

                for (var v = 0; v < triangle.vertices.Length; v++)
                {
                ...
                var iCount = triangle.vertices.Length;
                for (var i = 0; i < iCount; i++) triangles[count + i] = count + i;
        ...
        public class Legacy
        {
            public static Mesh2DSubmesh CreateBox(float size)
            {
                var result = new Mesh2DSubmesh(6);
