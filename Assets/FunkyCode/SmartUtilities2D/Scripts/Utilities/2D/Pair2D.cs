...
namespace FunkyCode.Utilities
{
    public class Pair2D
    {
        ...
        {
            var pairsList = new List<Pair2D>();
            if (list.Count > 0)
            {
                Vector2D p0 = null;
                if (connect) p0 = list.Last();
                ...
                for (var i = 0; i < list.Count; i++)
                {
                    ...
                    if (p0 != null) pairsList.Add(new Pair2D(p0, p1));
        ...
        public static List<Pair2D> GetList(Vector2[] list, bool connect = true)
        {
            var pairsList = new List<Pair2D>();
            if (list.Length > 0)
            {
                ...
                if (connect) p0 = list[list.Length - 1];
                ...
                for (var i = 0; i < list.Length; i++)
                {
                    ...
                    if (p0 != null) pairsList.Add(new Pair2D(p0.Value, p1));
