...
public class ManyLives生生不息 : Effect
{
    public int Count = 0;
    ...
    public void AddCount()
    {
        ...
        if (Count == 5)
        {
            if (mSelf.mData.CardId == 1018 ||
                mSelf.mData.CardId == 105)
                return;
            
            mSelf.PlayFlipAnimation(103);
        }

        if (mSelf.mData.CardId == 1018)
        {
            mSelf.PlayFlipAnimation(105);
        }
        else if (mSelf.mData.CardId == 105)
        {
            mSelf.PlayFlipAnimation(1018);
        }
    }
